        ...
        success: function (result) {
            ...
            if (result.status.name === "ok") {
                ...
                for (var i = 0; i < result['features'].length; i++) {
                    ...
                    if (result['features'][i].properties.ADMIN === country) {
						
                        bounds = result['features'][i];
						console.log(bounds);
                        break;
...
function displayInfo() {
    ...
    // to the modal.
	var flag = setFlag(countryAlpha2Code);
...
function setFlag(count) {
				$('#flag').html("<img src='https://www.countryflags.io/"+count+"/shiny/64.png'>");
			}
